...
void configuration_impl::load_service_tls(const boost::property_tree::ptree& _tree, std::shared_ptr<service>& _service) {
    try {
        for (const auto& kv : _tree) {
            ...
            } else if (key == "version-min") {
                tls_extra_[_service->unicast_address_][_service->reliable_]["min"] = val;
            } else if (key == "version-max") {
                tls_extra_[_service->unicast_address_][_service->reliable_]["max"] = val;
            } else if (key == "ciphersuites13") {
                tls_extra_[_service->unicast_address_][_service->reliable_]["c13"] = val;
            } else if (key == "ciphers") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["c12"] = val;
            } else if (key == "groups") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["grp"] = val;
            } else if (key == "sigalgs") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["sig"] = val;
            } else if (key == "early-data") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["ead"] = val;
            } else if (key == "key-derivation") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["kdf"] = val;
            } else if (key == "hash") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["hsh"] = val;
            } else if (key == "message-auth") {
                tls_extra_[_service->unicast_address_][_service->reliable_] ["mac"] = val;
            }
        }
    } catch (...) {
...
// Advanced TLS crypto parameter implementations for experimentation
std::string configuration_impl::get_tls_key_derivation_for_endpoint(const std::string& _address, std::uint16_t _port) const {
    std::lock_guard<std::mutex> its_lock(services_mutex_);
    auto fa = tls_extra_.find(_address);
    if (fa != tls_extra_.end()) {
        auto fp = fa->second.find(_port);
        if (fp != fa->second.end()) {
            auto fk = fp->second.find("kdf");
            if (fk != fp->second.end()) return fk->second;
        }
    }
    return {};
}

std::string configuration_impl::get_tls_hash_for_endpoint(const std::string& _address, std::uint16_t _port) const {
    std::lock_guard<std::mutex> its_lock(services_mutex_);
    auto fa = tls_extra_.find(_address);
    if (fa != tls_extra_.end()) {
        auto fp = fa->second.find(_port);
        if (fp != fa->second.end()) {
            auto fk = fp->second.find("hsh");
            if (fk != fp->second.end()) return fk->second;
        }
    }
    return {};
...
std::string configuration_impl::get_tls_message_auth_for_endpoint(const std::string& _address, std::uint16_t _port) const {
    std::lock_guard<std::mutex> its_lock(services_mutex_);
    auto fa = tls_extra_.find(_address);
    if (fa != tls_extra_.end()) {
        auto fp = fa->second.find(_port);
        if (fp != fa->second.end()) {
            auto fk = fp->second.find("mac");
            if (fk != fp->second.end()) return fk->second;
        }
    }
    return {};
