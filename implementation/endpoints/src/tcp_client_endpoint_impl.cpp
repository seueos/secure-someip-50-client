    ...
    if (!its_error || its_error == boost::asio::error::already_open) {
        ...
        if (tls_enable && std::string(tls_enable) == "1") {
            try {
                ...
                const char* ciphers = std::getenv("VSOMEIP_TLS_CIPHERS");          // TLS 1.2 and below
                const char* ciphers13 = std::getenv("VSOMEIP_TLS_CIPHERSUITES13"); // TLS 1.3
                const char* groups = std::getenv("VSOMEIP_TLS_GROUPS");            // e.g. "X25519:P-256"
                const char* sigalgs = std::getenv("VSOMEIP_TLS_SIGALGS");          // e.g. "ed25519:ecdsa_secp256r1_sha256"
                const char* early_data = std::getenv("VSOMEIP_TLS_EARLY_DATA");    // "0" to disable
...
#if defined(OPENSSL_VERSION_NUMBER)
                // Disable legacy protocols by default
...
#  if OPENSSL_VERSION_NUMBER >= 0x10100000L
                // Apply OpenSSL native settings when available
...
#    endif
                    if (ciphers && std::strlen(ciphers)) {
                        // Apply legacy cipher list if explicitly provided
