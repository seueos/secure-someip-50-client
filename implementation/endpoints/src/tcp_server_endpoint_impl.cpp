...
#include <string>
#include <fstream>
// TLS fine-tuning via OpenSSL native APIs
...
tcp_server_endpoint_impl::tcp_server_endpoint_impl(const std::shared_ptr<endpoint_host>& _endpoint_host,
    ...
    tcp_server_endpoint_base_impl(_endpoint_host, _routing_host, _io, _configuration), acceptor_(_io),
...
bool tcp_server_endpoint_impl::send_queued(const target_data_iterator_type _it) {
    ...
    connection::ptr its_connection;
    {
        ...
        if (connection_iterator != connections_.end()) {
            ...
            VSOMEIP_INFO << "Didn't find connection: " << _it->first.address().to_string() << ":" << std::dec
                         << static_cast<std::uint16_t>(_it->first.port()) << " dropping outstanding messages (" << std::dec
...
void tcp_server_endpoint_impl::accept_cbk(connection::ptr _connection, boost::system::error_code const& _error) {
    ...
    if (!_error) {
        ...
        endpoint_type remote;
        {
            ...
            if (its_error) {
                VSOMEIP_WARNING << "tsei::" << __func__ << ": couldn't enable keep_alive: " << its_error.message();
            }
            ...
            if (tls_enable && std::string(tls_enable) == "1") {
                try {
                    ...
                    // Enforce TLS versions and cipher suites if provided via environment
#if defined(OPENSSL_VERSION_NUMBER)
...
#  if OPENSSL_VERSION_NUMBER >= 0x10100000L
                    ...
                    if (native) {
                        ...
                        const char* groups = std::getenv("VSOMEIP_TLS_GROUPS");
                        const char* sigalgs = std::getenv("VSOMEIP_TLS_SIGALGS");
                        const char* early_data = std::getenv("VSOMEIP_TLS_EARLY_DATA");
                        // Advanced TLS crypto parameters for experimentation
                        const char* key_derivation = std::getenv("VSOMEIP_TLS_KEY_DERIVATION");
                        const char* hash = std::getenv("VSOMEIP_TLS_HASH");
                        const char* message_auth = std::getenv("VSOMEIP_TLS_MESSAGE_AUTH");
                        if (tls_min) {
                            if (!std::strcmp(tls_min, "TLS1_3")) SSL_CTX_set_min_proto_version(native, TLS1_3_VERSION);
                            else if (!std::strcmp(tls_min, "TLS1_2")) SSL_CTX_set_min_proto_version(native, TLS1_2_VERSION);
                        }
                        if (tls_max) {
                            if (!std::strcmp(tls_max, "TLS1_3")) SSL_CTX_set_max_proto_version(native, TLS1_3_VERSION);
                            else if (!std::strcmp(tls_max, "TLS1_2")) SSL_CTX_set_max_proto_version(native, TLS1_2_VERSION);
                        }
#    if OPENSSL_VERSION_NUMBER >= 0x10101000L
                        if (ciphers13 && std::strlen(ciphers13)) {
                            SSL_CTX_set_ciphersuites(native, ciphers13);
                        }
                        if (groups && std::strlen(groups)) {
                            SSL_CTX_set1_groups_list(native, groups);
                        }
                        if (sigalgs && std::strlen(sigalgs)) {
                            SSL_CTX_set1_sigalgs_list(native, sigalgs);
                        }
                        if (early_data && std::strcmp(early_data, "0") == 0) {
                            SSL_CTX_set_max_early_data(native, 0);
                        }

                        // Advanced TLS crypto parameters for experimentation
                        // Note: These are for logging and future extension purposes
                        // Actual implementation may require OpenSSL customizations
                        if (key_derivation && std::strlen(key_derivation)) {
                            // Log key derivation setting for experimentation
                            VSOMEIP_INFO << "TLS Key Derivation configured: " << key_derivation;
                        }
                        if (hash && std::strlen(hash)) {
                            // Log hash setting for experimentation
                            VSOMEIP_INFO << "TLS Hash configured: " << hash;
                        }
                        if (message_auth && std::strlen(message_auth)) {
                            // Log message authentication setting for experimentation
                            VSOMEIP_INFO << "TLS Message Auth configured: " << message_auth;
                        }
#    endif
                        if (ciphers && std::strlen(ciphers)) {
                            SSL_CTX_set_cipher_list(native, ciphers);
                        }
                    }
#  endif
#endif
    ...
    if (_error != boost::asio::error::bad_descriptor && _error != boost::asio::error::operation_aborted
        ...
        VSOMEIP_ERROR << "tcp_server_endpoint_impl::accept_cbk: " << _error.message() << " (" << std::dec << _error.value()
...
tcp_server_endpoint_impl::connection::connection(const std::weak_ptr<tcp_server_endpoint_impl>& _server, std::uint32_t _max_message_size,
    ...
    buffer_shrink_threshold_(_buffer_shrink_threshold), remote_port_(0), magic_cookies_enabled_(_magic_cookies_enabled),
            ...
            if (missing_capacity_) {
                ...
                if (its_capacity < its_required_capacity) {
                    ...
                    if (recv_buffer_.size() > 1048576) {
                        VSOMEIP_INFO << "tse: recv_buffer size is: " << recv_buffer_.size() << " local: " << get_address_port_local()
...
void tcp_server_endpoint_impl::connection::stop() {
    ...
    if (socket_.is_open()) {
        ...
        if (its_error) {
            VSOMEIP_WARNING << "tcp_server_endpoint_impl::connection::stop<" << get_address_port_remote()
                            << ">:shutting down socket failed (" << its_error.message() << ")";
        }
...
void tcp_server_endpoint_impl::connection::receive_cbk(boost::system::error_code const& _error, std::size_t _bytes) {
                ...
                if (!has_full_message) {
                    if (recv_buffer_size_ > VSOMEIP_RETURN_CODE_POS
                        ...
                        if (recv_buffer_[its_iteration_gap + VSOMEIP_PROTOCOL_VERSION_POS] != VSOMEIP_PROTOCOL_VERSION) {
                            {
                                std::lock_guard<std::mutex> its_lock(socket_mutex_);
                                VSOMEIP_ERROR << "tse: Wrong protocol version: 0x" << std::hex << std::setfill('0') << std::setw(2)
                            ...
                            its_host->on_message(&recv_buffer_[its_iteration_gap], VSOMEIP_SOMEIP_HEADER_SIZE + 8, its_server.get(), false,
                                           ...
                                           static_cast<message_type_e>(recv_buffer_[its_iteration_gap + VSOMEIP_MESSAGE_TYPE_POS]))) {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR << "tse: Invalid message type: 0x" << std::hex << std::setfill('0') << std::setw(2)
                                          ...
                                          << ". Closing connection due to missing/broken data TCP "
                                           ...
                                           static_cast<return_code_e>(recv_buffer_[its_iteration_gap + VSOMEIP_RETURN_CODE_POS]))) {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR << "tse: Invalid return code: 0x" << std::hex << std::setfill('0') << std::setw(2)
                        ...
                        {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR << "tse::c<" << this << ">rcb: recv_buffer_size is: " << std::dec << recv_buffer_size_
                                          << " but couldn't read "
    ...
    if (_error == boost::asio::error::eof || _error == boost::asio::error::connection_reset || _error == boost::asio::error::timed_out) {
        if (_error == boost::asio::error::timed_out) {
            ...
            VSOMEIP_WARNING << "tcp_server_endpoint receive_cbk: " << _error.message() << " local: " << get_address_port_local()
                            << " remote: " << get_address_port_remote();
        }
        wait_until_sent(boost::asio::error::operation_aborted);
    }
...
void tcp_server_endpoint_impl::connection::handle_recv_buffer_exception(const std::exception& _e) {
    ...
    its_message << "tcp_server_endpoint_impl::connection catched exception" << _e.what() << " local: " << get_address_port_local()
...
std::size_t tcp_server_endpoint_impl::connection::write_completion_condition(const boost::system::error_code& _error,
    ...
    if (_error) {
        VSOMEIP_ERROR << "tse::write_completion_condition: " << _error.message() << "(" << std::dec << _error.value()
    ...
    if (passed > send_timeout_warning_) {
        if (passed > send_timeout_) {
            ...
            VSOMEIP_WARNING << "tse::write_completion_condition: " << _error.message() << "(" << std::dec << _error.value()
...
void tcp_server_endpoint_impl::print_status() {
    ...
    for (const auto& c : its_connections) {
        ...
        VSOMEIP_INFO << "status tse: client: " << c.second->get_address_port_remote() << " queue: " << std::dec << its_queue_size
                     << " data: " << std::dec << its_data_size << " recv_buffer: " << std::dec << its_recv_size;
    }
}
...
void tcp_server_endpoint_impl::connection::wait_until_sent(const boost::system::error_code& _error) {
    ...
    if (it != its_server->targets_.end()) {
        ...
        if (its_data.is_sending_ && _error) {
            ...
            VSOMEIP_WARNING << __func__ << ": Maximum wait time for send operation exceeded for tse."
                            << " local: " << get_address_port_local() << " remote: " << get_address_port_remote();
        }
    }
    {
