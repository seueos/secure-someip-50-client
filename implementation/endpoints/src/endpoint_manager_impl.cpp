            ...
            if (its_tmp) {
                ...
                if (!its_error) {
                    ...
                    if (configuration_->is_tls_enabled_for_endpoint(its_unicast_str, _port)) {
                        ...
                        // TLS extras set via environment by runner script, not from configuration

                        // Advanced TLS crypto parameters for experimentation
                        auto key_derivation = configuration_->get_tls_key_derivation_for_endpoint(its_unicast_str, _port);
                        auto hash = configuration_->get_tls_hash_for_endpoint(its_unicast_str, _port);
                        auto message_auth = configuration_->get_tls_message_auth_for_endpoint(its_unicast_str, _port);
                        if (!key_derivation.empty()) setenv("VSOMEIP_TLS_KEY_DERIVATION", key_derivation.c_str(), 1);
                        if (!hash.empty()) setenv("VSOMEIP_TLS_HASH", hash.c_str(), 1);
                        if (!message_auth.empty()) setenv("VSOMEIP_TLS_MESSAGE_AUTH", message_auth.c_str(), 1);
                    }
    ...
    try {
        if (_reliable) {
            ...
            if (configuration_->is_tls_enabled_for_endpoint(_address.to_string(), _remote_port)) {
                ...
                // Advanced TLS crypto parameters for experimentation
                auto key_derivation = configuration_->get_tls_key_derivation_for_endpoint(_address.to_string(), _remote_port);
                auto hash = configuration_->get_tls_hash_for_endpoint(_address.to_string(), _remote_port);
                auto message_auth = configuration_->get_tls_message_auth_for_endpoint(_address.to_string(), _remote_port);
                if (!key_derivation.empty()) setenv("VSOMEIP_TLS_KEY_DERIVATION", key_derivation.c_str(), 1);
                if (!hash.empty()) setenv("VSOMEIP_TLS_HASH", hash.c_str(), 1);
                if (!message_auth.empty()) setenv("VSOMEIP_TLS_MESSAGE_AUTH", message_auth.c_str(), 1);
            }
